...
class Mario extends Player {
  ...
  void setupStates(String spriteSet, String type) {
    ...
    // Looking State
    State looking = new State("looking","graphics/"+spriteSet+"/"+type+"/Looking-mario.gif");
    addState(looking);
    
    //Spinning State
     State spinning = new State("spinning","graphics/"+spriteSet+"/"+type+"/Spinning-mario.gif",1,4);
     spinning.setDuration(15);
     addState(spinning);
  ...
  void handleMarioInput() {
    ...
    if (active.name!="crouching" && (isKeyDown('Q') || isKeyDown('D'))) {
      if (isKeyDown('Q')) {
    ...
    // if the spinning key is pressed, and we're standing on something, let's jump! 
    if (active.mayChange() && isKeyDown('X') && boundaries.size()>0) {
      ignore('X');
    ...
    if (active.mayChange() && isKeyDown('Z') && boundaries.size()>0) {
      ignore('Z');
      // generate a massive impulse upward
      addImpulse(0, -35);
      // and make sure we look like we're jumping, too
  ...
  void overlapOccurredWith(Actor other, float[] direction) {
    if (other instanceof MarioEnemy) {
      ...
      if (PI/2 - tolerance <= angle && angle <= PI/2 + tolerance) {
        ...
        if (spriteSet == "mario" &&spriteSet == "spinning") {
          setCurrentState("spinning");
        }
        else if (spriteSet == "mario" &&spriteSet == "jumping"){
          setCurrentState("jumping");
        }
      }
